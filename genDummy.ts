...
import { idsModel, type iIds } from "./schemas/models/ids.js";
...
for (let d = 0; d < 7; d++) {
    // 7 hari ke belakang
    const timeSet: Record<string, boolean> = {};
    ...
    for (let i = 0; i < n; i++) {
        const iotReadings: iIoTPayload["readings"] = {
            control: random(0, 31),
            pH: random(5, 9),
            tds: random(0, 600),
        ...
        const rand = random(
            1,
            d === 6 ? (Date.now() - getMidnightDate(new Date()).getTime()) / 2 : 16 * 3600 * 1000
        );
        do {
...
await new idsModel({ notificationId: notificationId } as iIds).save();
