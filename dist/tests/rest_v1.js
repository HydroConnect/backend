...
import { usageNotificationsModel, zUsageNotification, } from "../schemas/models/usageNotifications.js";
import { devicesModel } from "../schemas/models/devices.js";
import { idsModel } from "../schemas/models/ids.js";
import { zPanduanData } from "../schemas/panduanData.js";
...
beforeAll(async () => {
    const readingsData = JSON.parse(readFileSync(path.resolve(__dirname, "samples", "readingsTest.json")).toString("utf-8"));
    const summariesData = JSON.parse(readFileSync(path.resolve(__dirname, "samples", "summariesTest.json")).toString("utf-8"));
...
describe("GET /summary", () => {
    ...
    it("Implements paging", async () => {
        const data = JSON.parse((await myaxios.get("/summary?end_date=" + (Date.now() - 3600 * 70 * 1000).toString()))
            .data);
        console.log(data);
        expect(() => {
            for (let i = 0; i < data.length; i++) {
        ...
        expect(data[0].timestamp).toEqual(getMidnightDate(new Date(Date.now() - 3600 * 70 * 1000)).toISOString());
...
describe("POST /readings", () => {
    it("Rejects Invalid Body", async () => {
        const { status } = await myaxios.post("/readings", JSON.stringify({
            invalid: "data",
        }));
...
describe("GET /notifications", () => {
    it("Get LIMIT notifications", async () => {
        ...
        expect(() => {
            zUsageNotification.parse(data[parseInt(process.env.USAGE_NOTIFICATION_PAGING_LIMIT) - 1]);
        }).not.toThrow();
    });
    it("Pages", async () => {
        const data1 = JSON.parse((await myaxios.get("/notifications?latest=")).data);
        const data2 = JSON.parse((await myaxios.get("/notifications?latest=" + data1[data1.length - 1].notificationId))
            .data);
...
describe("POST /notifications/register", () => {
    it("Rejects Invalid Body", async () => {
        const { status } = await myaxios.post("/notifications/register", JSON.stringify({
            token: "data",
        }));
        expect(status).toEqual(400);
    });
...
describe("POST /notifications/unregister", () => {
    it("Rejects Invalid Body", async () => {
        const { status } = await myaxios.post("/notifications/unregister", JSON.stringify({
            token: "data",
        }));
        expect(status).toEqual(400);
    });
...
describe("Send Notifications", () => {
    ...
    const limit = parseInt(process.env.IOT_INTERVAL_MS) + parseInt(process.env.IOT_INTERVAL_TOLERANCE_MS);
    it.todo("Change expo token to be equal in phone");
    it("Prepare for Notification Sending", () => {
        return new Promise((resolve) => {
            myaxios
                .post("/notifications/register", JSON.stringify({
                token: realExpoToken,
            }))
                .then(() => {
                setTimeout(async () => {
